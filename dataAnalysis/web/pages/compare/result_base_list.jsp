...
<script language="javascript">  
  	...
  	function showimport(){
  		 ...
  		 //param="action1=CHECKSPACE&tableId="+$NAME("query.parameters(tableId)")[0].value;
  		...
  		// }else
  		 ...
  		 if(checkIsNull("inpqueryname"))
  		  {	
  	  		...
  	  		$NAME("tableId")[0].value=$NAME("query.parameters(tableId)")[0].value;
  		 ...
  		 function showexport(){
  		 	var url = contextpath + "dynamicquery/queryCondition.do";
  		    var param="action1=GETPARAM&tableId="+$NAME("query.parameters(tableId)")[0].value+"&condType=3";
  		  	var loader=dhtmlxAjax.postSync(url,param);
  		 ...
  		 function exportparam(){
  		 	...
  		 	var url = fulllink+"?action=TOADVANCEDPAGE&condId="+$NAME("inpqueryname")[0].value;
  	...
  	function checkInput(expressctl){
  		...
  		for(i=0;i<ctrl.length;i++){
  			...
  			if((expressctl.indexOf(ctrl[i].id)==-1 || expressctl.indexOf(ctrl[i].name)==-1) && ctrl[i].value!='' && ctrl[i].type!='hidden')
  			{
  	  			isInput=true;
  			}
  		}
  		for(i=0;i<ctrl1.length;i++){
  			if(expressctl.indexOf(ctrl1[i].name)==-1 && ctrl1[i].value!='')
  				isInput=true;
  		}
  		if(!isInput){
  			alert("请至少需要输入一个查询条件或选择一个下拉框");
  			return false;
  		}else
  			return true;
  	}
		...
		<table width="100%" cellspacing="1" cellpadding="1" class="controlTable">
                 	<tr>
                  	<td class="textL" valign="top">
                  	   ...
                  	   请填写/选择查询条件
                  	   	&nbsp;
                  	<input type="radio" name="querymode" value="base" onclick="toQuery('base')" checked="checked" />基本查询&nbsp;
                  	...
                  	<td class="textR">
                  	<gw:button styleClass="sbuBtnStyle" onClick="showexport()" title="从个人空间中导入查询条件" icon="openIcon">打开</gw:button>
                  	<gw:button styleClass="sbuBtnStyle" onClick="showimport()" title="保存当前查询条件到个人空间中" icon="saveIcon">保存</gw:button>		
                 	</td></tr>
                 	      </table>
                 	    </td>  
               		 </tr>	
      ...
      <tbody>
         ...
         <tr class="btnTr">
           ...
           <gw:button styleClass="btnStyle" code="" onClick="goQuery()">查询</gw:button>
              &nbsp;
           <gw:button styleClass="btnStyle" code="" onClick="reSet()">重置</gw:button>
           </td>
         </tr>
        </tbody>
      </table>               	
